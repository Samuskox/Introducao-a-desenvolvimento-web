...
function TipoList() {
    ...
    const handleDelete = (id) => {
        if (!window.confirm(`Tem certeza que deseja excluir o Tipo ID ${id}?`)) return;
        fetch(`/api/v1/tipos/${id}`, { method: 'DELETE' })
            .then(resp => {
    ...
    if (loading) return <p>Carregando tipos...</p>;

    return (
        <div>
            <h2>Lista de Tipos</h2>
            <button onClick={() => navigate('/tipo/novo')}>+ Adicionar Novo Tipo</button>

            {items.length === 0 ? (
                <p>Nenhum tipo encontrado no banco de dados.</p>
            ) : (
                <table>
                    <thead>
                        <tr>
