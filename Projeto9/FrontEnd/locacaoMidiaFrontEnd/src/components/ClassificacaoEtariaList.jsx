...
function ClassificacaoEtariaList() {
    ...
    const handleDelete = (id) => {
        if (!window.confirm(`Tem certeza que deseja excluir a Classificação ID ${id}?`)) return;
        fetch(`/api/v1/classificacao-etaria/${id}`, { method: 'DELETE' })
            .then(resp => {
    ...
    const handleEditSubmit = (e) => {
        ...
        fetch(`/api/v1/classificacao-etaria/${editingId}`, {
            method: 'PUT',
    ...
    return (
        <div>
            <h2>Lista de Classificações Etárias</h2>
            <button onClick={() => navigate('/classificacaoEtaria/novo')}>+ Adicionar Nova Classificação</button>
...
export default ClassificacaoEtariaList;
